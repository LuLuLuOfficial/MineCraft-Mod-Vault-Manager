...
def Mod_Search(Mod_Name: str,
    ...
    facets: list = deepcopy(Params["facets"]); Params["facets"] = ''
    facets.append([f'versions:{Project_Info["Game_Version"]}'])
    facets.append([f'categories:{Project_Info["Mods_Loader"]}'])

    Params["facets"] = '['
    for Part in facets:
        Params["facets"] += f'["{Part[0]}"],'
    if Params["facets"][-1] == ',': Params["facets"] = Params["facets"][:-1]
    Params["facets"] += ']'
        
    API_Search = 'https://api.modrinth.com/v2/search'
    response: Response = get(url=API_Search, timeout=5, params=Params)
    if response.status_code != 200:
...
def Mod_Locate(Mod_ID: str, Project_Info: dict, Cribble: bool = False) -> dict:
    '''
    Mod_ID: str = {Mod_ID}
    Project_Info: dict = {"Game_Version": '游戏版本',
                          "Mods_Loader": 'Mod加载器版本',
...
if __name__ == '__main__':
    ...
    pprint.pprint(Mod_Search(Mod_Name='carpet', Project_Info={"Game_Version": '1.21.1', "Mods_Loader": 'fabric'}))
